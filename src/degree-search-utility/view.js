...
export default function View() {
	...
	const [ collegeFilter, setCollegeFilter ] = useState(
		new URLSearchParams( window.location.search ).get( 'college' ) || ''
	);
				...
				const formattedPrograms = data.map( ( program ) => {
					...
					const concentrations =
						program._embedded?.[ 'wp:term' ]
							?.flat()
							.filter(
								( term ) => term.taxonomy === 'concentration'
							)
							.map( ( concentration ) => ( {
								id: concentration.id,
								name: concentration.name.replace(
									' (Online)',
									''
								),
	...
	useEffect( () => {
		const match = areaMap.find(
			( obj ) => obj.id === parseInt( areaFilter )
		);
		if ( match && match.name ) {
	...
	useEffect( () => {
		const match = collegeMap.find(
			( obj ) => obj.id === parseInt( collegeFilter )
		);
		if ( match && match.name ) {
				...
				<div className="programs-filters alignwide">
							...
							<div class="form-floating">
								...
								>
									...
									<option
										...
										value="Undergraduate"
										selected={
											degreeTypeFilter === 'Undergraduate'
												? true
												: false
										}
									>
					...
					<div
						...
						className={ `programs-filters-sticky${
							isSticky ? ' programs-filters-sticky--fixed' : ''
						}` }
					>
						{ ( searchTerm.length > 0 ||
					...
					<div
						className="programs-filters-results"
						id="program-results"
					>
						{ isLoading ? (
								...
								{ programs.map( ( program ) => (
									...
									>
										<div className="program-entry-block">
											<p className="program-entry-label">
												Program
											</p>
											<p className="program-entry-text program-entry-text--bold">
												{ program.major }
											</p>
										</div>
										<div className="program-entry-block">
